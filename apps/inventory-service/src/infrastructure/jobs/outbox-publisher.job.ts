...
import { KAFKA_TOPIC_INVENTORY } from '@orderflow/event-contracts';
...
export class OutboxPublisherJob {
    ...
    constructor(
        ...
        this.producer = this.kafka.producer({
            ...
            transactionalId: `inventory-service-outbox-${process.env.HOSTNAME || 'unknown'}`,
        });
    }

    async onModuleInit() {
        await this.producer.connect();
        this.logger.log('Inventory outbox publisher producer connected');
    }
    ...
    async publishOutboxEvents(): Promise<void> {
      ...
      `;
            ...
            this.logger.debug(`Publishing ${unpublishedEvents.length} inventory outbox events`);
            ...
            for (const event of unpublishedEvents) {
                try {
                    ...
                    this.logger.debug('Inventory outbox event published', {
                        eventId: event.event_id,
                        eventType: event.event_type,
                        aggregateId: event.aggregate_id,
                    });
                } catch (error) {
                    this.logger.error('Failed to publish inventory outbox event', (error as Error).stack || String(error));
                    // Will retry on next poll
                }
            }

            this.logger.log(`Successfully published ${unpublishedEvents.length} inventory outbox events`);
        } catch (error) {
            this.logger.error('Inventory outbox publisher job failed', (error as Error).stack || String(error));
        }
    }
    ...
    async cleanupOldOutboxEvents(): Promise<void> {
        try {
            ...
            this.logger.log(`Cleaned up ${result.count} old inventory outbox events`);
        } catch (error) {
            this.logger.error('Inventory outbox cleanup job failed', (error as Error).stack || String(error));
        }
    }
